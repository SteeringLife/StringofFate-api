...
describe 'Test Tag Search Handling' do
  ...
  describe 'Getting cards by tag' do
    before do
      @account.add_owned_card(DATA[:cards][0])
      @wrong_account.add_owned_card(DATA[:cards][1])
      @private_hashtag = StringofFate::CreatePrivateHashtagToCardForOwner.call(
        auth: { account: @account },
        card_id: @account.cards.first.id,
        private_hashtag_data: DATA[:private_hashtags][0]
      )
      @public_hashtag = PublicHashtag.create(DATA[:public_hashtags][0])
      @wrong_account.cards.first.add_public_hashtag(public_hashtag)
    end

    it 'HAPPY: should get cards list by tags for authorized account' do
      header 'AUTHORIZATION', auth_header(@account_data)
      get "api/v1/cards/tags/#{@private_hashtag.content}"
