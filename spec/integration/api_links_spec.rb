...
describe 'Test Link Handling' do
  ...
  it 'HAPPY: should be able to get list of all links' do
    ...
    DATA[:links].each do |link|
      acct.add_owned_link(link)
    end

    get "api/v1/accounts/#{acct.username}/links"
  ...
  it 'HAPPY: should be able to get details of a single link' do
    ...
    acct = StringofFate::Account.first
    link = acct.add_owned_link(link_data)

    get "/api/v1/accounts/#{acct.username}/links/#{link.id}"
    _(last_response.status).must_equal 200
  ...
  it 'SAD: should return error if unknown link requested' do
    acct = StringofFate::Account.first
    get "/api/v1/accounts/#{acct.username}/links/foobar"
  ...
  it 'SECURITY: should prevent basic SQL injection targeting IDs' do
    acct = StringofFate::Account.first
    acct.add_owned_link(DATA[:links][0])
    acct.add_owned_link(DATA[:links][1])
    get "/api/v1/accounts/#{acct.username}/links/2%20or%20id%3E0"
  ...
  #     req_header = { 'CONTENT_TYPE' => 'application/json' }
  #     post "api/v1/accounts/#{@acct.username}/links",
